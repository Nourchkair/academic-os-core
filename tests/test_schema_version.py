...
def test_checked_in_schema_conforms_to_canonical_config_contract() -> None:
    ...
    assert set(schema["required"]) >= {"schema_version", "student", "academic", "runtime", "acquisition", "privacy", "browser", "agents"}
    assert "automation" not in schema["required"]
    assert "integrations" not in schema["properties"]
    assert schema["properties"]["legacy_compatibility"]["deprecated"] is True
