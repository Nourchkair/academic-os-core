...
- **Agent-first operating layer:** `academia agent context|attention|changes|capabilities --json` composes existing workspace, domain, task, Library, Review, Activity, processing, provenance, and capability services into bounded stable JSON. Scopes are workspace/semester/course/today and details are compact/standard/deep; source content is never dumped automatically.
- **Conversation-facing Review contract:** `agent attention` translates every unresolved Review item into a question, why human input is needed, current/proposed values, evidence, explicit choices, action proposal, and status. The existing Review decision/execution/verification workflow remains authoritative.
...
- **Safe generated artifacts:** `artifact create` is create-only, Markdown/text-first, source-linked for source-derived kinds, provenance-enforced as `AI-GENERATED`, and writes only to lazy `<semester>/<course>/06_KNOWLEDGE/AI_GENERATED/`. `.academia/artifacts.json` and Activity provide registry/audit visibility, and Library/dashboard identify these files as AI-created.
- **Small observational dashboard update:** Home exposes the existing Review count as “Needs input”; Courses exposes an explicit AI-created material category/badge; no chatbot or major Review workflow was added.
