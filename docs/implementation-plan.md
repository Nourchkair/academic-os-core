...
- **Small observational dashboard update:** Home labels the existing Review count as “Open decisions”; Courses exposes an explicit AI-created material category/badge; Settings includes a low-clutter “Enhance your setup” recommendation surface with no external connection controls; no chatbot or major Review workflow was added.
- **Optional workflow registry:** `academia_os/recommendations.py` validates nine machine-readable reference recipes exposed through read-only `academia agent recommendations` and `academia agent recipe`; recipes describe external-agent outcomes and safety without claiming integrations or storing their state.
- **Student workflow preferences:** `academia_os/workflow_preferences.py` stores validated local overrides, custom instructions, optional external setup notes, and audit attribution under the academic workspace. `academia workflow preferences/show/set` and the Settings editor share this state; saved preferences never prove or authorize external connections.
