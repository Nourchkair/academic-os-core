...
metadata:
  hermes:
    tags: [academics, university, study, provenance, assignments, exams, calendar, gmail, research]
    related_skills: [google-workspace, himalaya, ocr-and-documents]
---
...
For a durable academic automation, prefer a cheap pre-run gate over waking an agent on every poll. The gate should inventory all active-course `00_INBOX/` files, ignore harmless metadata such as `.DS_Store` and `.gitkeep`, compare path/size/mtime/inode signatures against state stored outside the academic root under the active Hermes home, and print a final `{"wakeAgent": false}` when nothing changed or `{"wakeAgent": true, ...}` with changed-file context when work exists. Persist the current signature before waking to suppress repeated processing of an unchanged uncertain file. Pair this with an agent job that follows the non-destructive inbox protocol and a daily brief job that reads current course state, modality, Calendar/Gmail when available, deadlines, workload, weak areas, and study strategies; writes the latest generated dashboard to the active semester's `TODAY.md`; and delivers only to the configured primary target. Verify the scheduler timezone, workdir, job state, integration access, empty-gate path, new-file wake path, and repeat suppression path with a temporary test artifact.

## Brightspace Acquisition Layer

Brightspace retrieval is on-demand acquisition, separate from inbox processing. The reusable browser verifier must distinguish the browser's human-facing profile label from its actual Chromium user-data/profile directory: record both, and require `real_profile: true` plus `clean_hermes_profile: false`. A successful attachment to a browser labelled `Work` may still use the `Default` profile directory; do not treat that naming difference as a profile mismatch when the verifier confirms the same user-data root.

After retrieval, verify the page itself—not only the URL or browser login state—contains the exact course code/title and requested week/module. Treat the exact module boundary as the request scope: do not silently include a separate course-wide module such as “Lecture Slides” merely because it may relate to the same week. If the course log already records a successful retrieval for that exact module, verify the live module contents and local hashes first; do not redownload identical files. Record a no-op re-verification when appropriate, including the module’s exact topic count, unchanged Downloads result, and any intentionally excluded adjacent modules. Preserve the direct topic URL and stable Brightspace identifiers in the source log/index when available, because these are stronger provenance anchors than a generic course landing page.
 For commands such as `Fetch Brightspace — [course] — Week [N]`, first run the profile-aware preflight `python3 ~/.hermes/scripts/academic_os_brightspace_browser.py ensure`. It must pin and verify the active supported Chromium-family browser profile used for the configured institution, verify Hermes CDP is attached to that same real user-data directory/profile, and refuse the clean Hermes `~/.hermes/chrome-debug` profile. Then verify the exact the configured institution course by code/title/semester/instructor and resolve the requested week/module against current course context before downloading. Use the authenticated visible local browser when downloads are required. Never request, type, save, or record passwords, MFA/Duo codes, or other credentials; if authentication is needed, stop at the the institution sign-in screen and let the user authenticate manually. After the user reports completion, rerun the preflight and verify the same real profile before resuming. Treat Brightspace as read-only: do not submit assignments, take/answer quizzes, post messages, change settings, intentionally mark work complete, delete content, unenroll, or change account settings.

Snapshot the local Downloads directory before browser downloads. Afterward, identify only files newly created or changed by that retrieval, filter to relevant academic documents, and hand them to the exact course `00_INBOX` without manual sorting. A local handoff utility may hash-check the course, skip identical material, preserve changed versions with collision-safe names, and record a concise retrieval log. Important non-downloadable pages may be captured as clearly labelled Brightspace-content Markdown with title, course, module, retrieval date, URL, and source type. After the handoff, the existing Inbox Processor owns classification and all downstream updates. If course/module identity, authentication, or download status is uncertain, stop and report rather than guess. Reaching a the institution sign-in login page proves neither course access nor retrieval: record the blocked attempt, verify that the target inbox is unchanged, and only claim success after exact course/week evidence and a Downloads diff confirm the handoff. See `references/brightspace-acquisition.md` for the deterministic sequence and verification expectations.
