...
function Home({ status, tasks, activity, domain, inbox, onNavigate }: { status: StatusPayload | null; tasks: Task[]; activity: ActivityEvent[]; domain: DomainEntity[]; inbox: ProcessingRecord[]; onNavigate: (view: View) => void }) {
  ...
  return <div className="page-stack">
    ...
    <div className="metric-strip"><Metric label="Courses" value={String(status?.workspace.courses.length ?? 0)} /><Metric label="Material" value={String(materialCount)} /><Metric label="Open tasks" value={String(openTasks.length)} /><Metric label="Open decisions" value={String(status?.review_count ?? 0)} /><Metric label="Intake" value={String(pendingInbox.length)} /></div>
