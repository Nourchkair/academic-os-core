  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {error ? <ConnectionNotice message={error} onRetry={() => void refresh()} /> : <>{view === 'home' && <Home status={status} tasks={tasks} reviews={reviews} activity={activity} domain={domain} onNavigate={setView} />}{view === 'courses' && <Courses courses={courses} domain={domain} />}{view === 'tasks' && <Tasks tasks={tasks} />}{view === 'library' && <Library courses={courses} onNavigate={setView} />}{view === 'import' && status && <ImportView status={status} courses={courses} onImported={refresh} />}{view === 'review' && <Review reviews={reviews} activity={activity} courses={courses} actionBusy={actionBusy} onReviewDecision={async (decision, itemId) => { try { setActionBusy(itemId); setError(null); await api.reviewDecision(itemId, decision); await refresh() } catch (reason) { setError(String(reason)) } finally { setActionBusy(null) } }} />}{view === 'settings' && <Settings status={status} onSaved={refresh} />}</>}
      </main>
    </div>
  )
}
